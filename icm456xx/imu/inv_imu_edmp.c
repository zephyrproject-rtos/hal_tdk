...
int inv_imu_edmp_init_apex(inv_imu_device_t *s)
{
	...
	/*
	 * Initialize read and write pointers for pedometer to buffer full condition. EDMP will
	 * always	 * write step count in EDMP_PED_STEP_CNT_BUF2. Initialize read and write
	 * pointers for freefall to 0.
	 */
...
int inv_imu_edmp_get_apex_parameters(inv_imu_device_t *s, inv_imu_edmp_apex_parameters_t *p)
{
	...
	status |=
		INV_IMU_READ_EDMP_SRAM(s, EDMP_BASICSMD_WIN_WAIT, (uint8_t *)&p->basicsmd_win_wait);
#else
	/* SMD */
...
int inv_imu_edmp_set_apex_parameters(inv_imu_device_t *s, const inv_imu_edmp_apex_parameters_t *p)
{
	...
	status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_BASICSMD_WIN_WAIT,
					  (uint8_t *)&p->basicsmd_win_wait);
#else
	/* SMD */
