...
#include "icm456xx/imu/inv_imu_edmp_defs.h"
...
int icm456xx_edmp_set_frequency(inv_imu_device_t *s, const dmp_ext_sen_odr_cfg_apex_odr_t frequency)
{
	...
	status |= icm456xx_read_reg(s, DMP_EXT_SEN_ODR_CFG, 1, (uint8_t *)&dmp_ext_sen_odr_cfg);
	dmp_ext_sen_odr_cfg.apex_odr = frequency;
	status |= icm456xx_write_reg(s, DMP_EXT_SEN_ODR_CFG, 1, (uint8_t *)&dmp_ext_sen_odr_cfg);
...
int icm456xx_edmp_init_apex(inv_imu_device_t *s)
{
	...
	status |= icm456xx_edmp_configure(s);
	 ...
	 */
	status |= icm456xx_read_reg(s, APEX_BUFFER_MGMT, 1, (uint8_t *)&apex_buffer_mgmt);
	...
	status |= icm456xx_write_reg(s, APEX_BUFFER_MGMT, 1, (uint8_t *)&apex_buffer_mgmt);

	/* Same impl as icm456xx_adv_power_up_sram, duplicated here to prevent dependency */
	...
	for (int i = 0; i < EDMP_ROM_DATA_SIZE; i++)
		status |= icm456xx_write_sram(s, (uint32_t)EDMP_RAM_BASE + i, 1, &value);

	status |= icm456xx_edmp_recompute_apex_decimation(s);
...
int icm456xx_edmp_recompute_apex_decimation(inv_imu_device_t *s)
{
	 ...
	 */
	status |= icm456xx_read_reg(s, EDMP_APEX_EN0, 1, (uint8_t *)&save_edmp_apex_en0);
	status |= icm456xx_read_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&save_edmp_apex_en1);
	 ...
	 */
	value = 0x3F;
	status |= icm456xx_write_reg(s, STATUS_MASK_PIN_0_7, 1, &value);
	status |= icm456xx_write_reg(s, STATUS_MASK_PIN_8_15, 1, &value);
	status |= icm456xx_write_reg(s, STATUS_MASK_PIN_16_23, 1, &value);

	/* Trigger EDMP with on-demand mode */
	status |= icm456xx_edmp_unmask_int_src(s, INV_IMU_EDMP_INT0, EDMP_INT_SRC_ON_DEMAND_MASK);

	/*
	 * Request to execute init procedure, make sure init is the only feature enabled
	 * (overwrite previously saved config)
	 */
	status |= icm456xx_write_reg(s, EDMP_APEX_EN0, 1, (uint8_t *)&edmp_apex_en0);
	edmp_apex_en1.init_en     = INV_IMU_ENABLE;
	edmp_apex_en1.edmp_enable = INV_IMU_ENABLE;
	status |= icm456xx_write_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&edmp_apex_en1);

	status |= icm456xx_read_reg(s, REG_HOST_MSG, 1, (uint8_t *)&reg_host_msg);
	reg_host_msg.edmp_on_demand_en = INV_IMU_ENABLE;
	status |= icm456xx_write_reg(s, REG_HOST_MSG, 1, (uint8_t *)&reg_host_msg);

	/* Wait 200 us to give enough time for EMDP to start running */
	icm456xx_sleep_us(s, 200);

	/* Wait for DMP execution to complete */
	status |= icm456xx_edmp_wait_for_idle(s);

	/* Reset states */
	status |= icm456xx_edmp_mask_int_src(s, INV_IMU_EDMP_INT0, EDMP_INT_SRC_ON_DEMAND_MASK);

	/* Restore original DMP state, with DMP necessarily disabled as it was checked at the beginning of this function */
	status |= icm456xx_write_reg(s, EDMP_APEX_EN0, 1, (uint8_t *)&save_edmp_apex_en0);
	status |= icm456xx_write_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&save_edmp_apex_en1);

	status |= icm456xx_edmp_unmask_int_src(s, INV_IMU_EDMP_INT0, EDMP_INT_SRC_ACCEL_DRDY_MASK);
...
int icm456xx_edmp_get_apex_parameters(inv_imu_device_t *s, inv_imu_edmp_apex_parameters_t *p)
{
	...
	status |= icm456xx_read_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&edmp_apex_en1);
...
int icm456xx_edmp_set_apex_parameters(inv_imu_device_t *s, const inv_imu_edmp_apex_parameters_t *p)
{
	...
	status |= icm456xx_read_reg(s, EDMP_APEX_EN0, 2, (uint8_t *)&cfg);
	...
	status |= icm456xx_read_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&cfg.edmp_apex_en1);
	cfg.edmp_apex_en1.power_save_en = p->power_save_en;
	status |= icm456xx_write_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&cfg.edmp_apex_en1);
...
int icm456xx_edmp_get_config_int_apex(inv_imu_device_t *s, inv_imu_edmp_int_state_t *it)
{
	...
	status |= icm456xx_read_reg(s, INT_APEX_CONFIG0, 2, (uint8_t *)&cfg);
	/* INT_APEX_CONFIG0 */
...
int icm456xx_edmp_set_config_int_apex(inv_imu_device_t *s, const inv_imu_edmp_int_state_t *it)
{
	...
	status |= icm456xx_write_reg(s, INT_APEX_CONFIG0, 2, (uint8_t *)&cfg);
...
/*
 * Check if icm456xx_edmp_set_frequency() was called without recomputing APEX decimation
 * thanks to icm456xx_edmp_recompute_apex_decimation().
 ...
 * Returns INV_IMU_ERROR_EDMP_ODR if icm456xx_edmp_recompute_apex_decimation() should have been
 * called.
 */
static int check_dmp_odr_decimation(inv_imu_device_t *s)
{
	...
	status |= icm456xx_read_sram(s, 0x460 /* edmp decimation rate address in SRAM */, 1,
...
int icm456xx_edmp_enable_pedometer(inv_imu_device_t *s)
{
	...
	status |= icm456xx_read_reg(s, EDMP_APEX_EN0, 1, (uint8_t *)&edmp_apex_en0);
...
int icm456xx_edmp_get_int_apex_status(inv_imu_device_t *s, inv_imu_edmp_int_state_t *it)
{
	...
	status |= icm456xx_read_reg(s, INT_APEX_STATUS0, 2, (uint8_t *)&st);
...
int icm456xx_edmp_get_pedometer_data(inv_imu_device_t *s, inv_imu_edmp_pedometer_data_t *ped_data)
{
	int     status = INV_IMU_OK;
	uint8_t data[2];
	uint8_t retry = 0;
	 ...
	 * configuration in `icm456xx_edmp_init_apex` function.
	 */
...
int icm456xx_edmp_mask_int_src(inv_imu_device_t *s, inv_imu_edmp_int_t edmp_int_nb, uint8_t int_mask)
{
	int      status = INV_IMU_OK;
	uint32_t reg_addr;
	uint8_t  reg;
	...
	status |= icm456xx_read_reg(s, reg_addr, 1, &reg);
	reg |= int_mask;
	status |= icm456xx_write_reg(s, reg_addr, 1, &reg);
...
int icm456xx_edmp_configure(inv_imu_device_t *s)
{
	...
	status |=
	    icm456xx_write_reg(s, EDMP_PRGRM_IRQ0_0, sizeof(start_addr), (uint8_t *)&start_addr[0]);

	/* Set Stack pointer start address */
	status |= icm456xx_write_reg(s, EDMP_SP_START_ADDR, sizeof(stack_addr), (uint8_t *)&stack_addr);
...
int icm456xx_edmp_run_ondemand(inv_imu_device_t *s, inv_imu_edmp_int_t edmp_int_nb)
{
	...
	status |= icm456xx_edmp_unmask_int_src(s, edmp_int_nb, EDMP_INT_SRC_ON_DEMAND_MASK);

	status |= icm456xx_edmp_enable(s);
...
int icm456xx_edmp_wait_for_idle(inv_imu_device_t *s)
{
	...
	while (status == INV_IMU_OK) {
		status |= icm456xx_read_reg(s, IPREG_MISC, 1, (uint8_t *)&ipreg_misc);
		if (ipreg_misc.edmp_idle != 0)
			break;

		icm456xx_sleep_us(s, 5);
		timeout_us -= 5;
