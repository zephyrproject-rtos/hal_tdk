 ...
 * steps Range: [0-15] Default value: 5 Recommendation: set same value as ped_step_cnt_th.
 */
 ...
 * number Range: [0 - 225] * Default value: 150 for ODR = 50 Hz Recommendation: it is linked to stop
 * and go use case to avoid seeing the impact of pedo_step_cnt_thr and pedo_step_det_thr
 */
 ...
 * may increase Range: 0: Normal 1: Slow walk Default: 0
 */
 ...
 * Unit: 4*number of samples between two consecutive steps. Cadency (step/s) = (ped_step_cadence /
 * 4) / (pedometer_ ODR).
 */
 ...
 * quat_reset_en
 *
 * Set 1 to force reset 3-axis quaternion when next tilt reset is done. This is applicable only if
 * tilt_reset_en is also set to 1. Unit: N/A Range: [0 - 1] Default: 0
 */
 ...
 * reduce false positives but reduce detection rate (especially for transport use cases). Unit: N/A
 * Ranging: [0 - 4]
 * Default: 0
 */
 ...
 * Unit: time in sample number
 * Range: [25 - 500] * Default value: 150
 ...
 * Unit: N/A
 * Range: 3 lower bits only are used [b2 b1 b0]: * -	b2 = 1 swap X and Y
 * -	b1 = 1 flip X sign
 * -	b0 = 1 flip Y sign
 ...
 *
 * Gain used to filter the accelerometer to obtain an estimation of the gravity (low-pass filter),
 * defined as : forgetting factor = Gain * SAMPLING_PERIOD / (40 * 32), and 100Hz. Range: [2-16]
 ...
 * motion is Y axis up (with less than 30 degrees of inclination). Unit: ms (no dependency on ODR,
 * it is managed internally by the algorithm) Range: [100 - 500] Default: 240
 */
 ...
 *
 * Timer relative to the rapidity of the algorithm to trigger wake up when the orientation before
 * motion is over 30 degrees on the Y axis Unit: ms (no dependency on ODR, it is managed internally
 * by the algorithm) Range: [240- 1000] Default: 500
 */
 ...
 *
 * Time delay to update gravity in case motion is detected all the time, force to update gravity
 * estimator even if the device is not stable. Unit: ms (no dependency on ODR, it is managed
 * internally by the algorithm) Range: [1000 - 10000] Default: 2600
 */
 ...
 * highg_time_th
 *
 * The number of samples device should stay above (highg_peak_th + highg_peak_th_hyst) before HighG
 * state is triggered. Unit: time in samples number Range: [1-300] Default: 1 (set for default ODR =
 * 800 Hz, equivalent to 1.25 ms)
 */
 ...
 *
 * Period after a freefall is signaled during which a new freefall will not be detected. Prevents
 * false detection due to bounces. Unit: time in samples number Range: [75 - 3000] Default: 800 (set
 * for default ODR = 800 Hz, equivalent to 1 s)
 */
 ...
 * tap_max_peak_tol
 *
 * Maximum peak tolerance is the percentage of pulse amplitude to get the smudge threshold for
 * rejection. Unit: N/A Range: [1 (12.5%) 2 (25.0%) 3 (37.5%) 4 (50.0 %)] Default: 2
 */
 ...
 *
 * max acceptable number of samples (jerk value) over  tap_max_peak_tol the during the Tmin window.
 * Over this value, Tap event is rejected unit: time in number of samples range: [13 â€“ 92]
 ...
 * double_tap_timing
 *
 * In case of double tap, indicate the sample count between the two detected pulses. Double tap
 * timing in seconds is double_tap_timing / ACCEL_ODR_Hz.
 */
 ...
 * Unit: time in sample number
 * Range: [0 - 4294967295] * Default value: 6400
 */
 ...
 * bit0: If set, enable self-test init, must be set when any of accel or gyro self-test enable bit
 * is set (bits 2:1) bit1: If set, enable accel self-test * bit2: If set, enable gyro self-test
 * bit7~9: Averaging time used to perform self-test (0/1/2/3/4/5: 10/20/40/80/160/320 ms)
 * bit10~12: Tolerance between factory trim and accel self-test response (0/1/2/3/4/5/6/7:
 * 5/10/15/20/25/30/40/50%) bit13~15: Tolerance between factory trim and gyro self-test response
 * (0/1/2/3/4/5/6/7: 5/10/15/20/25/30/40/50%)
 */
