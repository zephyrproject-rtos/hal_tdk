...
int icm456xx_adv_init(inv_imu_device_t *s)
{
	...
	status |= icm456xx_get_who_am_i(s, &whoami);
	if (whoami != INV_IMU_WHOAMI)
		return INV_IMU_ERROR;

	/* Reset device */
	status |= icm456xx_adv_device_reset(s);

	return status;
}

int icm456xx_adv_device_reset(inv_imu_device_t *s)
{
	int                status = INV_IMU_OK;
	inv_imu_adv_var_t *e      = (inv_imu_adv_var_t *)s->adv_var;

	status |= icm456xx_soft_reset(s);

	/* Read endianness for further processing */
	status |= icm456xx_get_endianness(s); /* Set `endianness_data` variable */
...
int icm456xx_adv_enable_accel_lp(inv_imu_device_t *s)
{
	return icm456xx_set_accel_mode(s, PWR_MGMT0_ACCEL_MODE_LP);
}

int icm456xx_adv_enable_accel_ln(inv_imu_device_t *s)
{
	return icm456xx_set_accel_mode(s, PWR_MGMT0_ACCEL_MODE_LN);
}

int icm456xx_adv_disable_accel(inv_imu_device_t *s)
{
	int                      status = INV_IMU_OK;
	...
	/* Turn off accel */
	status |= icm456xx_set_accel_mode(s, PWR_MGMT0_ACCEL_MODE_OFF);

	if (clear_fifo) {
		/* Reset FIFO to guarantee no data is left after turning off all sensors. */
		status |= icm456xx_adv_reset_fifo(s);
	}

	return status;
}

int icm456xx_adv_enable_gyro_ln(inv_imu_device_t *s)
{
	return icm456xx_set_gyro_mode(s, PWR_MGMT0_GYRO_MODE_LN);
}

int icm456xx_adv_enable_gyro_lp(inv_imu_device_t *s)
{
	return icm456xx_set_gyro_mode(s, PWR_MGMT0_GYRO_MODE_LP);
}

int icm456xx_adv_disable_gyro(inv_imu_device_t *s)
{
	int                      status = INV_IMU_OK;
	const inv_imu_adv_var_t *e      = (const inv_imu_adv_var_t *)s->adv_var;
	pwr_mgmt0_t              pwr_mgmt0;
	...
	/* Turn off gyro */
	status |= icm456xx_set_gyro_mode(s, PWR_MGMT0_GYRO_MODE_OFF);
...
int icm456xx_adv_set_int2_pin_usage(inv_imu_device_t *                             s,
                                   ioc_pad_scenario_ovrd_pads_int2_cfg_ovrd_val_t usage)
{
	...
	status |=
	    icm456xx_read_reg(s, IOC_PAD_SCENARIO_AUX_OVRD, 1, (uint8_t *)&ioc_pad_scenario_aux_ovrd);
	ioc_pad_scenario_aux_ovrd.aux2_enable_ovrd     = 1;
	ioc_pad_scenario_aux_ovrd.aux2_enable_ovrd_val = 0;
	status |=
	    icm456xx_write_reg(s, IOC_PAD_SCENARIO_AUX_OVRD, 1, (uint8_t *)&ioc_pad_scenario_aux_ovrd);
#endif

	status |= icm456xx_read_reg(s, IOC_PAD_SCENARIO_OVRD, 1, (uint8_t *)&ioc_pad_scenario_ovrd);
	ioc_pad_scenario_ovrd.pads_int2_cfg_ovrd     = 1;
	ioc_pad_scenario_ovrd.pads_int2_cfg_ovrd_val = (uint8_t)usage;
	status |= icm456xx_write_reg(s, IOC_PAD_SCENARIO_OVRD, 1, (uint8_t *)&ioc_pad_scenario_ovrd);
...
#if INV_IMU_FSYNC_SUPPORTED
int icm456xx_adv_configure_fsync_ap_tag(inv_imu_device_t *s, fsync_config0_ap_fsync_sel_t sensor_tag)
{
	int                status = INV_IMU_OK;
	inv_imu_adv_var_t *e      = (inv_imu_adv_var_t *)s->adv_var;
	...
	status |= icm456xx_read_reg(s, FSYNC_CONFIG0, 1, (uint8_t *)&fsync_config0);
	fsync_config0.ap_fsync_sel = (uint8_t)sensor_tag;
	status |= icm456xx_write_reg(s, FSYNC_CONFIG0, 1, (uint8_t *)&fsync_config0);

	return status;
}

int icm456xx_adv_enable_fsync(inv_imu_device_t *s)
{
	int               status = INV_IMU_OK;
	tmst_wom_config_t tmst_wom_config;
	smc_control_0_t   smc_control_0;

	status |= icm456xx_read_reg(s, TMST_WOM_CONFIG, 1, (uint8_t *)&tmst_wom_config);
	tmst_wom_config.tmst_delta_en = INV_IMU_ENABLE;
	status |= icm456xx_write_reg(s, TMST_WOM_CONFIG, 1, (uint8_t *)&tmst_wom_config);
...
int icm456xx_adv_disable_fsync(inv_imu_device_t *s)
{
	int               status = INV_IMU_OK;
	smc_control_0_t   smc_control_0;
	tmst_wom_config_t tmst_wom_config;
...
int icm456xx_adv_configure_wom(inv_imu_device_t *s, const uint8_t wom_x_th, const uint8_t wom_y_th,
                              ...
                              tmst_wom_config_wom_int_dur_t wom_dur)
{
	...
	status |= icm456xx_write_reg(s, ACCEL_WOM_X_THR, sizeof(data), &data[0]);
...
int icm456xx_adv_enable_wom(inv_imu_device_t *s)
{
	int               status = INV_IMU_OK;
	tmst_wom_config_t tmst_wom_config;
...
int icm456xx_adv_disable_wom(inv_imu_device_t *s)
{
	int               status = INV_IMU_OK;
	tmst_wom_config_t tmst_wom_config;
...
int icm456xx_adv_get_data_from_registers(inv_imu_device_t *s)
{
	int                      status = INV_IMU_OK;
	const inv_imu_adv_var_t *e      = (const inv_imu_adv_var_t *)s->adv_var;
	...
	status |= icm456xx_read_reg(s, ACCEL_DATA_X1_UI, sizeof(data), data);
	...
	if (fsync_tag_in_accel || fsync_tag_in_gyro || fsync_tag_in_temp) {
		 ...
		 */
		status |= icm456xx_read_reg(s, TMST_FSYNCH, 2, fsync_count);
...
int icm456xx_adv_get_data_from_fifo(inv_imu_device_t *s, uint8_t fifo_data[FIFO_MIRRORING_SIZE],
                                   uint16_t *fifo_count)
{
	int                      status = INV_IMU_OK;
	const inv_imu_adv_var_t *e      = (const inv_imu_adv_var_t *)s->adv_var;
	...
	/* Read FIFO count */
	status |= icm456xx_get_frame_count(s, fifo_count);
	...
	/* Read FIFO data */
	status |= icm456xx_read_reg(s, FIFO_DATA, *fifo_count * s->fifo_frame_size, fifo_data);
	return status;
}

int icm456xx_adv_parse_fifo_data(inv_imu_device_t *s, const uint8_t fifo_data[FIFO_MIRRORING_SIZE],
...
uint32_t icm456xx_adv_convert_odr_bitfield_to_us(uint32_t odr_bitfield)
{
	switch (odr_bitfield) {
...
int icm456xx_adv_get_accel_fsr(inv_imu_device_t *s, accel_config0_accel_ui_fs_sel_t *accel_fsr)
{
	int                      status = INV_IMU_OK;
	const inv_imu_adv_var_t *e      = (const inv_imu_adv_var_t *)s->adv_var;
	fifo_config3_t           fifo_config3;

	status |= icm456xx_read_reg(s, FIFO_CONFIG3, 1, (uint8_t *)&fifo_config3);
...
int icm456xx_adv_get_gyro_fsr(inv_imu_device_t *s, gyro_config0_gyro_ui_fs_sel_t *gyro_fsr)
{
	int                      status = INV_IMU_OK;
	const inv_imu_adv_var_t *e      = (const inv_imu_adv_var_t *)s->adv_var;
	fifo_config3_t           fifo_config3;
...
int icm456xx_adv_set_timestamp_resolution(inv_imu_device_t *                 s,
...
int icm456xx_adv_reset_fifo(inv_imu_device_t *s)
{
	int status = INV_IMU_OK;

	status |= icm456xx_flush_fifo(s);
	status |= init_fifo_compression(s);

	return status;
}

int icm456xx_adv_get_fifo_config(inv_imu_device_t *s, inv_imu_adv_fifo_config_t *conf)
{
	int                   status = INV_IMU_OK;
	fifo_configx_t        cfg;
	...
	if (cfg.fifo_config0.fifo_depth == FIFO_CONFIG0_FIFO_DEPTH_MAX ||
		 ...
		 * It will eventually be applied when calling `icm456xx_set_fifo_config`.
		 */
	...
	status |= icm456xx_read_reg(s, ODR_DECIMATE_CONFIG, 1, (uint8_t *)&odr_decimate_config);
...
int icm456xx_adv_set_fifo_config(inv_imu_device_t *s, const inv_imu_adv_fifo_config_t *conf)
{
	int                   status = INV_IMU_OK;
	inv_imu_adv_var_t *   e      = (inv_imu_adv_var_t *)s->adv_var;
	fifo_configx_t        cfg;
	...
	if (cfg.fifo_config4.fifo_comp_en == INV_IMU_ENABLE) {
		...
		if (pwr_mgmt0.accel_mode != PWR_MGMT0_ACCEL_MODE_OFF) {
			accel_config0_t accel_config0;
			status |= icm456xx_read_reg(s, ACCEL_CONFIG0, 1, (uint8_t *)&accel_config0);
			accel_odr = icm456xx_adv_convert_odr_bitfield_to_us(accel_config0.accel_odr);
		}

		if (pwr_mgmt0.gyro_mode != PWR_MGMT0_GYRO_MODE_OFF) {
			gyro_config0_t gyro_config0;
			status |= icm456xx_read_reg(s, GYRO_CONFIG0, 1, (uint8_t *)&gyro_config0);
			gyro_odr = icm456xx_adv_convert_odr_bitfield_to_us(gyro_config0.gyro_odr);
		}

		cfg.fifo_config4.fifo_comp_en = INV_IMU_DISABLE;
		status |= icm456xx_write_reg(s, FIFO_CONFIG4, 1, (uint8_t *)&cfg.fifo_config4);

		/* Wait 2 ODR */
		if (accel_odr != UINT32_MAX || gyro_odr != UINT32_MAX)
			icm456xx_sleep_us(s, 2 * (accel_odr < gyro_odr ? accel_odr : gyro_odr));
	}
	...
	status |= icm456xx_write_reg(s, ODR_DECIMATE_CONFIG, 1, (uint8_t *)&odr_decimate_config);
...
uint32_t icm456xx_adv_get_timestamp_resolution_us(inv_imu_device_t *s)
{
	int               status = INV_IMU_OK;
	tmst_wom_config_t tmst_wom_config;
...
int icm456xx_adv_enable_clkin_rtc(inv_imu_device_t *s)
{
	int                  status = INV_IMU_OK;
	 ...
	 */
	status |= icm456xx_read_reg(s, SIFS_I3C_STC_CFG, 1, (uint8_t *)&sifs_i3c_stc_cfg);
	sifs_i3c_stc_cfg.i3c_stc_mode = INV_IMU_DISABLE;
	status |= icm456xx_write_reg(s, SIFS_I3C_STC_CFG, 1, (uint8_t *)&sifs_i3c_stc_cfg);

	/* Same as in I3CSM STC mode, the ACCEL_SRC_CTRL[1:0] and GYRO_SRC_CTRL[1:0]
	 * must be set to 2’b10 (FIR and interpolator on)
	 */
	status |= icm456xx_read_reg(s, IPREG_SYS2_REG_123, 1, (uint8_t *)&ipreg_sys2_reg_123);
	ipreg_sys2_reg_123.accel_src_ctrl = IPREG_SYS2_REG_123_ACCEL_SRC_CTRL_INTERPOLATOR_ON_FIR_ON;
	status |= icm456xx_write_reg(s, IPREG_SYS2_REG_123, 1, (uint8_t *)&ipreg_sys2_reg_123);

	status |= icm456xx_read_reg(s, IPREG_SYS1_REG_166, 1, (uint8_t *)&ipreg_sys1_reg_166);
	ipreg_sys1_reg_166.gyro_src_ctrl = IPREG_SYS1_REG_166_GYRO_SRC_CTRL_INTERPOLATOR_ON_FIR_ON;
	status |= icm456xx_write_reg(s, IPREG_SYS1_REG_166, 1, (uint8_t *)&ipreg_sys1_reg_166);

	status |= icm456xx_read_reg(s, RTC_CONFIG, 1, (uint8_t *)&otp_heater_rtc_config);
	otp_heater_rtc_config.rtc_mode = INV_IMU_ENABLE;
	status |= icm456xx_write_reg(s, RTC_CONFIG, 1, (uint8_t *)&otp_heater_rtc_config);

	return status;
}

int icm456xx_adv_disable_clkin_rtc(inv_imu_device_t *s)
{
	int          status = INV_IMU_OK;
	rtc_config_t otp_heater_rtc_config;
...
int icm456xx_adv_power_up_sram(inv_imu_device_t *s)
{
	int               status = INV_IMU_OK;
	fifo_sram_sleep_t fifo_sram_sleep;

	status |= icm456xx_read_reg(s, FIFO_SRAM_SLEEP, 1, (uint8_t *)&fifo_sram_sleep);
	fifo_sram_sleep.fifo_gsleep_shared_sram = 3;
	status |= icm456xx_write_reg(s, FIFO_SRAM_SLEEP, 1, (uint8_t *)&fifo_sram_sleep);

	return status;
}

int icm456xx_adv_power_down_sram(inv_imu_device_t *s)
{
	int               status = INV_IMU_OK;
	fifo_sram_sleep_t fifo_sram_sleep;
...
int icm456xx_adv_set_endianness(inv_imu_device_t *s, sreg_ctrl_sreg_data_endian_sel_t endianness)
{
	int         status = INV_IMU_OK;
	...
	status |= icm456xx_write_reg(s, SREG_CTRL, 1, (uint8_t *)&sreg_ctrl);
