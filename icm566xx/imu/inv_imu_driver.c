...
int icm566xx_soft_reset(inv_imu_device_t *s)
{
	...
	status |= icm566xx_read_reg(s, INTF_CONFIG1_OVRD, 1, (uint8_t *)&intf_config1_ovrd);
	/* Save DRIVE_CONFIG0 register */
	status |= icm566xx_read_reg(s, DRIVE_CONFIG0, 1, (uint8_t *)&drive_config0);

	/* Trigger soft reset */
	reg_misc2.soft_rst = 1;
	status |= icm566xx_write_reg(s, REG_MISC2, 1, (uint8_t *)&reg_misc2);

	/* Wait 1 ms for soft reset to be effective */
	icm566xx_sleep_us(s, 1000);

	/* Restore DRIVE_CONFIG0 register */
	status |= icm566xx_write_reg(s, DRIVE_CONFIG0, 1, (uint8_t *)&drive_config0);
...
int icm566xx_get_who_am_i(inv_imu_device_t *s, uint8_t *who_am_i)
{
	int status;
...
int icm566xx_set_accel_mode(inv_imu_device_t *s, pwr_mgmt0_accel_mode_t accel_mode)
{
	int status = INV_IMU_OK;
...
int icm566xx_set_gyro_mode(inv_imu_device_t *s, pwr_mgmt0_gyro_mode_t gyro_mode)
{
	int status = INV_IMU_OK;
	pwr_mgmt0_t pwr_mgmt0;
...
int icm566xx_set_accel_frequency(inv_imu_device_t *s, const accel_config0_accel_odr_t frequency)
{
	int status = INV_IMU_OK;
...
int icm566xx_set_gyro_frequency(inv_imu_device_t *s, const gyro_config0_gyro_odr_t frequency)
{
	int status = INV_IMU_OK;
...
int icm566xx_set_accel_fsr(inv_imu_device_t *s, accel_config0_ap_accel_fs_sel_t accel_fsr)
{
	int status = INV_IMU_OK;
	accel_config0_t accel_config0;
...
int icm566xx_set_gyro_ln_bw(inv_imu_device_t *s, ipreg_sys1_reg_158_gyro_ui_lpfbw_sel_t gyr_bw)
{
	int status = INV_IMU_OK;
...
int icm566xx_set_gyro_lp_avg(inv_imu_device_t *s, ipreg_sys1_reg_157_gyro_lp gyr_avg)
{
	int status = INV_IMU_OK;
...
int icm566xx_set_accel_lp_avg(inv_imu_device_t *s, ipreg_sys2_reg_110_accel_lp_avg_sel_t acc_avg)
{
	int status = INV_IMU_OK;
...
int icm566xx_set_accel_ln_bw(inv_imu_device_t *s, ipreg_sys2_reg_112_accel_ui_lpfbw_t acc_bw)
{
	int status = INV_IMU_OK;
...
int icm566xx_set_gyro_notch_bypass(inv_imu_device_t *s, uint8_t bypass_gyr_notch)
{
	int status = INV_IMU_OK;
	ipreg_sys1_reg_157_t ipreg_sys1_reg_157;
...
int icm566xx_get_register_data(inv_imu_device_t *s, inv_imu_sensor_data_t *data)
{
	...
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[0],
			   (low_order_data[0] & 0x0F), &data->accel_data[0]);
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[2],
			   (low_order_data[1] & 0x0F), &data->accel_data[1]);
	FORMAT_SENSOR_DATA(s->endianness_data, (uint8_t *)&high_order_data[4],
	...
	FORMAT_16_BITS_DATA(s->endianness_data, (uint8_t *)&high_order_data[12], &data->temp_data);

	return status;
}

int icm566xx_set_fifo_config(inv_imu_device_t *s, const inv_imu_fifo_config_t *fifo_config)
{
	int status = INV_IMU_OK;
	fifo_configx_t cfg;
	...
	status |= icm566xx_write_reg(s, FIFO_CONFIG3, 1, (uint8_t *)&cfg.fifo_config3);
...
int icm566xx_get_fifo_config(inv_imu_device_t *s, inv_imu_fifo_config_t *fifo_config)
{
	int status = INV_IMU_OK;
	fifo_configx_t cfg;

	status |= icm566xx_read_reg(s, FIFO_CONFIG0, 5, (uint8_t *)&cfg);
	...
	if (cfg.fifo_config0.fifo_depth == FIFO_CONFIG0_FIFO_DEPTH_MAX ||
		 ...
		 * It will be eventually applied when calling `icm566xx_set_fifo_config`.
		 */
		fifo_config->fifo_depth = FIFO_CONFIG0_FIFO_DEPTH_APEX;
	}

	/* FIFO_CONFIG1_0 and FIFO_CONFIG1_1*/
	fifo_config->fifo_wm_th =
...
int icm566xx_flush_fifo(inv_imu_device_t *s)
{
	int status = INV_IMU_OK;
	fifo_config2_t fifo_config2;
	int timeout_us = 1000000; /* 1 sec */

	status |= icm566xx_read_reg(s, FIFO_CONFIG2, 1, (uint8_t *)&fifo_config2);
	fifo_config2.fifo_flush = INV_IMU_ENABLE;
	status |= icm566xx_write_reg(s, FIFO_CONFIG2, 1, (uint8_t *)&fifo_config2);
	icm566xx_sleep_us(s, 10);
...
int icm566xx_get_frame_count(inv_imu_device_t *s, uint16_t *frame_count)
{
	int status = INV_IMU_OK;

	status |= icm566xx_read_reg(s, FIFO_COUNT_0, 2, (uint8_t *)frame_count);
	FORMAT_16_BITS_DATA(s->endianness_data, (uint8_t *)frame_count, frame_count);

	return status;
}

int icm566xx_get_fifo_frame(inv_imu_device_t *s, inv_imu_fifo_data_t *data)
{
	int status = INV_IMU_OK;
	uint8_t frame[20];
	int16_t reg16[3];

	status |= icm566xx_read_reg(s, FIFO_DATA, s->fifo_frame_size, frame);
			    ...
			    const inv_imu_int_state_t *it)
{
	...
	status |= icm566xx_write_reg(s, reg, 2, (uint8_t *)&intx_configx);
			    ...
			    inv_imu_int_state_t *it)
{
	...
	status |= icm566xx_read_reg(s, reg, 2, (uint8_t *)&intx_configx);
			    ...
			    inv_imu_int_state_t *it)
{
	...
	status |= icm566xx_read_reg(s, reg, 2, (uint8_t *)&int_status);
...
int icm566xx_select_accel_lp_clk(inv_imu_device_t *s, pwr_mgmt_0_accel_lp_clk_sel_t clk_sel)
{
	int status = INV_IMU_OK;
	pwr_mgmt0_t pwr_mgmt0;
...
const char *icm566xx_get_version(void)
{
