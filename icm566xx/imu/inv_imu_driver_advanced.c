...
int icm566xx_adv_device_reset(inv_imu_device_t *s)
{
	...
	status |= icm566xx_get_endianness(s); /* Set `endianness_data` variable */
...
{
	return icm566xx_set_accel_mode(s, PWR_MGMT0_ACCEL_MODE_LP);
}

int icm566xx_adv_enable_accel_ln(inv_imu_device_t *s)
{
	return icm566xx_set_accel_mode(s, PWR_MGMT0_ACCEL_MODE_LN);
}
...
int icm566xx_adv_disable_accel(inv_imu_device_t *s)
{
	...
	/* Turn off accel */
	status |= icm566xx_set_accel_mode(s, PWR_MGMT0_ACCEL_MODE_OFF);

	if (clear_fifo) {
		/* Reset FIFO to guarrantee no data is left after turning off all sensors. */
		status |= icm566xx_adv_reset_fifo(s);
	}
...
{
	return icm566xx_set_gyro_mode(s, PWR_MGMT0_GYRO_MODE_LN);
}

int icm566xx_adv_enable_gyro_lp(inv_imu_device_t *s)
{
	return icm566xx_set_gyro_mode(s, PWR_MGMT0_GYRO_MODE_LP);
}
...
int icm566xx_adv_disable_gyro(inv_imu_device_t *s)
{
	...
	/* Turn off gyro */
	status |= icm566xx_set_gyro_mode(s, PWR_MGMT0_GYRO_MODE_OFF);
				    ...
				    ioc_pad_scenario_ovrd_pads_int2_cfg_ovrd_val_t usage)
{
	...
	ioc_pad_scenario_ovrd.pads_int2_cfg_ovrd_val = usage;
	status |=
					...
					fsync_config0_ap_fsync_sel_t sensor_tag)
{
	...
	fsync_config0.ap_fsync_sel = sensor_tag;
	status |= icm566xx_write_reg(s, FSYNC_CONFIG0, 1, (uint8_t *)&fsync_config0);
			       ...
			       tmst_wom_config_wom_int_dur_t wom_dur)
{
	...
	tmst_wom_config.wom_int_mode = wom_int;
	tmst_wom_config.wom_int_dur = wom_dur;
	status |= icm566xx_write_reg(s, TMST_WOM_CONFIG, 1, (uint8_t *)&tmst_wom_config);
...
static int parse_fifo_frame(inv_imu_device_t *s, uint8_t *frame)
{
	...
	event.sensor_mask = 0;
	header = (fifo_header_t *)&(frame[frame_idx]);
...
static int decode_compressed_event(inv_imu_device_t *s, uint8_t *frame, uint8_t event_num)
{
	int status = INV_IMU_OK;
	inv_imu_adv_var_t *e = (inv_imu_adv_var_t *)s->adv_var;
	fifo_comp_header_t *comp_header = (fifo_comp_header_t *)&frame[0];
	fifo_comp_decode_t *decode_header = (fifo_comp_decode_t *)&frame[1];
...
static int parse_compressed_fifo_frame(inv_imu_device_t *s, uint8_t *frame)
{
	...
	fifo_comp_header_t *header = (fifo_comp_header_t *)&frame[0];
	int event_num = 1;
				    ...
				    uint16_t fifo_data_size, uint16_t *fifo_count)
{
	...
	/* Read FIFO count */
	status |= icm566xx_read_reg(s, FIFO_COUNT_0, 2, &fifo_count_reg[0]);
	FORMAT_16_BITS_DATA(s->endianness_data, fifo_count_reg, fifo_count);
				 ...
				 const uint16_t fifo_count)
{
	...
	for (uint16_t i = 0; i < fifo_count; i++) {
		...
		if (e->fifo_comp_en) {
			fifo_header_t *header = (fifo_header_t *)&(frame[0]);
...
int icm566xx_adv_get_accel_fsr(inv_imu_device_t *s, accel_config0_ap_accel_fs_sel_t *accel_fsr)
{
	...
	if (e->fifo_is_used == INV_IMU_ENABLE && (fifo_config3.fifo_hires_en == INV_IMU_ENABLE)) {
		...
		*accel_fsr = ACCEL_CONFIG0_AP_ACCEL_FS_SEL_32_G;
#else
		*accel_fsr = ACCEL_CONFIG0_AP_ACCEL_FS_SEL_16_G;
#endif
	} else {
		accel_config0_t accel_config0;
		status |= icm566xx_read_reg(s, ACCEL_CONFIG0, 1, (uint8_t *)&accel_config0);
		*accel_fsr = (accel_config0_ap_accel_fs_sel_t)accel_config0.ap_accel_fs_sel;
	}
...
int icm566xx_adv_get_gyro_fsr(inv_imu_device_t *s, gyro_config0_ap_gyro_fs_sel_t *gyro_fsr)
{
	...
	if (e->fifo_is_used == INV_IMU_ENABLE && (fifo_config3.fifo_hires_en == INV_IMU_ENABLE)) {
		/* FIFO is used and High Resolution mode is enabled */
#if INV_IMU_HIGH_FSR_SUPPORTED
		*gyro_fsr = GYRO_CONFIG0_AP_GYRO_FS_SEL_4000_DPS;
#else
		*gyro_fsr = GYRO_CONFIG0_AP_GYRO_FS_SEL_2000_DPS;
#endif
	} else {
		gyro_config0_t gyro_config0;
		status |= icm566xx_read_reg(s, GYRO_CONFIG0, 1, (uint8_t *)&gyro_config0);
		*gyro_fsr = (gyro_config0_ap_gyro_fs_sel_t)gyro_config0.ap_gyro_fs_sel;
	}
					  ...
					  const tmst_wom_config_tmst_resol_t timestamp_resol)
{
	...
	status |= icm566xx_read_reg(s, TMST_WOM_CONFIG, 1, (uint8_t *)&tmst_wom_config);
	tmst_wom_config.tmst_resol = timestamp_resol;
	status |= icm566xx_write_reg(s, TMST_WOM_CONFIG, 1, (uint8_t *)&tmst_wom_config);
...
int icm566xx_adv_get_fifo_config(inv_imu_device_t *s, inv_imu_adv_fifo_config_t *conf)
{
	...
	if (cfg.fifo_config0.fifo_depth == FIFO_CONFIG0_FIFO_DEPTH_MAX ||
		 ...
		 * It will eventually be applied when calling `icm566xx_set_fifo_config`.
		 */
...
int icm566xx_adv_set_fifo_config(inv_imu_device_t *s, const inv_imu_adv_fifo_config_t *conf)
{
	...
	if (cfg.fifo_config4.fifo_comp_en == INV_IMU_ENABLE) {
		...
		if (pwr_mgmt0.gyro_mode != PWR_MGMT0_GYRO_MODE_OFF) {
			gyro_config0_t gyro_config0;
			status |= icm566xx_read_reg(s, GYRO_CONFIG0, 1, (uint8_t *)&gyro_config0);
			gyro_odr = icm566xx_adv_convert_odr_bitfield_to_us(gyro_config0.gyro_odr);
		}
	...
	/* Set FIFO depth */
	cfg.fifo_config0.fifo_depth = conf->base_conf.fifo_depth;
	...
	status |= icm566xx_write_reg(s, ODR_DECIMATE_CONFIG, 1, (uint8_t *)&odr_decimate_config);
...
int icm566xx_adv_enable_clkin_rtc(inv_imu_device_t *s)
{
	...
	ipreg_sys2_reg_109.accel_src_ctrl = IPREG_SYS2_REG_109_ACCEL_SRC_CTRL_SRC_ON_PREFILTER_ON;
	status |= icm566xx_write_reg(s, IPREG_SYS2_REG_109, 1, (uint8_t *)&ipreg_sys2_reg_109);

	status |= icm566xx_read_reg(s, IPREG_SYS1_REG_154, 1, (uint8_t *)&ipreg_sys1_reg_154);
	ipreg_sys1_reg_154.gyro_src_ctrl = IPREG_SYS1_REG_154_GYRO_SRC_CTRL_SRC_ON_PREFILTER_ON;
	status |= icm566xx_write_reg(s, IPREG_SYS1_REG_154, 1, (uint8_t *)&ipreg_sys1_reg_154);
